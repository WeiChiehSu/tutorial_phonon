...
GAMMA_CENTER = .TRUE.
DOS = .TRUE.
DOS_RANGE = 0 10 0.001
PDOS = 1, 2, 3, 4, 5, 6,
EOF

# DOS_RANGE:Total and partial density of states are drawn with some parameters.
#The example makes DOS be calculated from frequency=0 to 40 with 0.01 pitch.
 
# Print phonopy version or verify it's installed
echo -e "Using phonopy: $(type -p phonopy)"

# Run phonopy-load to generate the PDOS plot
echo "Running phonopy-load to generate PDOS plot..."
phonopy-load --readfc --config mesh.conf -p -s

eval $run 1> >(tee ${PBS_JOBID}.${PBS_JOBNAME}.out) 2> >(tee -a ${PBS_JOBID}.${PBS_JOBNAME}.out)

echo "PhonopyPDOS done"
